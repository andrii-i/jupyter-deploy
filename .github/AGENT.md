...
| `e2e-eks-oidc-pr.yml` | `pull_request_target` | Per-PR EKS E2E against the restored deployment (auto for org members/collaborators, `e2e-approval` gate for other authors; mutating job gated on `e2e-approval` for everyone; PR head SHA pinned) |
